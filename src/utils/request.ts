...
const RequestMethod: RequestFunc.request = (
  ...
  if (carryUserId) {
    const userId = useUserInfo.data?.userInfo?.userId;
    if (typeof userId !== "undefined" && typeof data === "object") {
      data["userId"] = userId;
    } else {
			if(useUserInfo.data?.userInfo?.token) {
				toast('用户失效,请重新登陆')
			}
    }
  }
  ...
  if (carryToken) {
    const token = useUserInfo.data?.token;
...
export const put = <R = any, T = any>(
  ...
  RequestMethod<R, T>(
	...
	//detele请求
export const detele = <R = any, T = any>(
  url: string,
  data?: T,
  config?: RequestFunc.RequestConfig
) =>
...
export const uploadImage = url => {
  return new Promise((resolve, reject) => {
    Taro.uploadFile({
      ...
      filePath: url,
			header: {
				AuthorizationApi:  useUserInfo.data?.token,
			},
      name: "file",
      success(res) {
        try {
